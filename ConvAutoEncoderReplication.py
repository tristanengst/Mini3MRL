...
def extract_each_class(dataset):
    ...
    for idx,t in enumerate(dataset.targets):
        if t in target2idxs:
            target2idxs[t].append(idx)
        else:
            target2idxs[t] = [idx]
    result = [dataset.data[target2idxs[t][0]] for t in sorted(target2idxs.keys())]
    print(len(result))
    return result
#   images = []
#   ITERATE = True
...
training_data = CustomCIFAR10(training_images, transforms=transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]))
validation_data = CustomCIFAR10(validation_images, transforms=transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]))
test_data = CustomCIFAR10(test_images, transforms=transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]))
...
class ConvolutionalAutoencoder():
  def __init__(self, autoencoder):
    ...
    print(self.network)
    print(self.network.decoder.linear[0].bias)
  ...
  def train(self, loss_function, epochs, batch_size, 
    ...
    for epoch in range(epochs):
      ...
      for images in tqdm(train_loader):
        ...
        print(images)
        print(self.network(images.to(device)))
        assert 0
...
log_dict = model.train(nn.MSELoss(),
