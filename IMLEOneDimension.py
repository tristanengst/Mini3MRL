...
def evaluate(model, data_tr, data_val, scheduler, args, cur_step, nxz_data_tr=None):
    ...
    # Generate images
    output_tr, mapped_noise_tr = ImageLatentDataset.generate_images(nxz_data_tr, model, args, noise_seed="dataset")
    ...
    wandb.log({
        ...
        "lr": scheduler.get_lr(),
        "train_step": cur_step,
        "images/tr": wandb.Image(output_tr),
        "mapped_noise_tr/tr": wandb.Image(mapped_noise_tr),
        "epoch": epoch,
    }, step=cur_step)
...
class ImageLatentDataset(Dataset):
    ...
    def generate_images(nxz_data, model, args, idxs=None, num_images=None,
        ...
        with torch.no_grad():
            ...
            y = model(x, num_z=100, seed=args.seed)
            x = torch.repeat_interleave(x, y.shape[0] // x.shape[0], dim=0)
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=x.detach().cpu().numpy(), y=y.detach().cpu().numpy(),
                mode='markers',
                marker=dict(
    ...
    def get_image_latent_dataset(model, loss_fn, dataset, args):
        ...
        with torch.no_grad():
            ...
            for idx,(x,y) in tqdm(enumerate(loader),
                ...
                images[start_idx:stop_idx] = y.cpu()
